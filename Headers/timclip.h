...
#define TimC_SW_Tick_Tock_(Stopwatch,TimerSelect,State) if(TimC.SW.Read.State_(Stopwatch,TimerSelect)){(State)=TimCStateUnknown;}else for((State)=TimC.SW.Toggle_(Stopwatch,TimerSelect);(State)==TimCStateRunning;(State)=TimC.SW.Toggle_(Stopwatch,TimerSelect))
	...
	const struct
	{
		...
		logical(_PL_ Reset_)(TIMC_RG _PL_ RandomGenerator);
		...
		const struct
		{
			...
			//＊Return value is 1 for success, 0 for failure.
			logical(_PL_ Byte_)(TIMC_RG _PL_ RandomGenerator,ADDRESS StateSelect,general _PL_ _R_ ArrayData,ADDRESS ArraySize);
			//TimClip : Random Permutation
			//＊Knuth shuffles algorithm is used.
			//＊An array whose length exceeds ((2^64)-1) is not allowed.
			//＊Return value is 1 for success, 0 for failure.
			logical(_PL_ Perm_)(TIMC_RG _PL_ RandomGenerator,ADDRESS StateSelect,address _PL_ _R_ Array,ADDRESS Length);
		}
		Rand;
		...
		const struct
		{
			...
			//＊Return value is 1 for success, 0 for failure.
			logical(_PL_ I32_)(TIMC_RG _PL_ RandomGenerator,ADDRESS StateSelect,inte_32 _PL_ _R_ Array,ADDRESS Length,INTE_32 Low,INTE_32 High);

			//TimClip : 32-bit Real Array of Uniform Distribution U[L, H)
			//＊Return value is 1 for success, 0 for failure.
			logical(_PL_ R32_)(TIMC_RG _PL_ RandomGenerator,ADDRESS StateSelect,real_32 _PL_ _R_ Array,ADDRESS Length,REAL_32 Low,REAL_32 High);
			//TimClip : 64-bit Real Array of Uniform Distribution U[L, H)
			//＊Return value is 1 for success, 0 for failure.
			logical(_PL_ R64_)(TIMC_RG _PL_ RandomGenerator,ADDRESS StateSelect,real_64 _PL_ _R_ Array,ADDRESS Length,REAL_64 Low,REAL_64 High);
		}
		Uni;
		...
		const struct
		{
			...
			//＊Return value is 1 for success, 0 for failure.
			logical(_PL_ R32_)(TIMC_RG _PL_ RandomGenerator,ADDRESS StateSelect,real_32 _PL_ _R_ Array,ADDRESS Length,REAL_32 Mean,REAL_32 Deviation);
			//TimClip : 64-bit Real Array of Gaussian Distribution N(μ, σ²)
			//＊Box-Muller transform is used.
			//＊Return value is 1 for success, 0 for failure.
			logical(_PL_ R64_)(TIMC_RG _PL_ RandomGenerator,ADDRESS StateSelect,real_64 _PL_ _R_ Array,ADDRESS Length,REAL_64 Mean,REAL_64 Deviation);
		}
		Gau;
	}
	RG;
