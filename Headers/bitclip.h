...
#define BitC_Reform_(S,T,SShp,StoTAxis,Dims) ((sizeof(*(S))==sizeof(*(T)))?(BitC.Reform_(S,T,SShp,StoTAxis,Dims,sizeof(*(S)))):(BitCNull))
		...
		{
			//BitClip : Endianness
			//£ªPointed value is 0x00 for big-endian, 0x01 for little-endian.
		...
		const struct
		{
			...
			const struct
			{
				...
				//£ªO[n]£½(I[n/8].bit[n%8])?(0xFF):(0x00)
				general(_PL_ D08_)(DATA_08 *_R_ I,data_08 *_R_ O,ADDRESS N);
				//BitClip : Bit Expansion to 16-bit Natural
				//£ªO[n]£½(I[n/8].bit[n%8])?(0xFFFF):(0x0000)
				general(_PL_ D16_)(DATA_08 *_R_ I,data_16 *_R_ O,ADDRESS N);
				//BitClip : Bit Expansion to 32-bit Natural
				//£ªO[n]£½(I[n/8].bit[n%8])?(0xFFFFFFFF):(0x00000000)
				general(_PL_ D32_)(DATA_08 *_R_ I,data_32 *_R_ O,ADDRESS N);
				//BitClip : Bit Expansion to 64-bit Natural
				//£ªO[n]£½(I[n/8].bit[n%8])?(0xFFFFFFFFFFFFFFFF):(0x0000000000000000)
				general(_PL_ D64_)(DATA_08 *_R_ I,data_64 *_R_ O,ADDRESS N);
	...
	const struct
	{
		...
		const struct
		{
			...
			//£ªThe number of binary paths must be same as the number of devices associated with the context.
			//£ª(BinaryPath) Example : { "../../Device 1/bitclip.obj", "../../Device 2/bitclip.obj", ... }
		...
		//£ªRequired (Buffer) size is 4¡¿((DeviceAddressBits)¡À8) bytes.
		//£ªReturn address is (Pin) for the same address bits between the host and the device, or (Buffer) otherwise.
		...
		const struct
		{
			...
			//BitClip : Array Reformation
			//£ªSourceAmount->S[dim] == TargetAmount->S[ReformingAxis->S[dim]] where dim£½0, 1, 2, 3.
			//£ªSource and Target's type sizes should be same.
			...
			{
				//BitClip : NOT Operation
				//£ªC and A's type sizes should be same.
				...
				//BitClip : Shift Operation
				//£ªC and A's types should be same.
				...
				//BitClip : AND Operation
				//£ªC and A's type sizes should be same.
				//£ªRequired (Mask) size is the type size of those to be calculated.
				...
				//BitClip : AND Operation
				//£ªC, A, and B's type sizes should be same.
